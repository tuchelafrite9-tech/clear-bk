// Shared email sending logic for client approval emails
// Used by both SendApprovalEmail (admin-triggered) and RequestAccountOpening (public)
...
export function buildApprovalHtml(clientPrenom: string, clientNom: string, clientEmail: string): string {
  return `<!DOCTYPE html>
...
export async function sendApprovalEmail(base44: any, clientEmail: string, clientPrenom: string, clientNom: string): Promise<string> {
  const fullName = `${clientPrenom || ''} ${clientNom || ''}`.trim();
  const html = buildApprovalHtml(clientPrenom, clientNom, clientEmail);
