...
async function sendWithResend({ to, subject, html }: EmailMessage): Promise<DeliveryResult> {
  ...
  if (!apiKey || !senderDomain) {
    throw new Error("La configuration Resend est incomplète");
  }
...
async function sendWithGmail(base44: any, { to, subject, html, senderEmail }: EmailMessage): Promise<DeliveryResult> {
  if (!senderEmail) throw new Error("Adresse d’expéditeur Gmail indisponible");

  const rawMessage = [
    `From: ClearBank <${senderEmail}>`,
    `To: ${to}`,
